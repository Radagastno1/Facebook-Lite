...
public class UsersDB : IUserData, IDataToObject<User, User>
{
    ...
    public User GetOne(int id, User user = null)
    {
        if (user != null)
        {
            try
    ...
    public List<User> GetInactive()
    {
        List<User> users = new();
        string query =
            "SELECT u.id as 'Id', DATE_ADD(u.date_inactive, interval 30 day) "
            + "as deletingdate FROM users u  WHERE DATE_ADD(u.date_inactive, interval 30 day) < CURRENT_DATE() "
            + "AND is_deleted = false;";
        try
        {
            using MySqlConnection con = new MySqlConnection(
                $"Server=localhost;Database=facebook_lite;Uid=root;Pwd=;Allow User Variables=true;"
            );
            users = con.Query<User>(query).ToList();
            return users;
        }
