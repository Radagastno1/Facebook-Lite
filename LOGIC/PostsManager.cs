...
public class PostsManager : IManager<Post, User>
{
    ...
    public PostsManager(IData<Post> postData, IDataToObject<Post, User> postObject, IDataToList<Post, User> postDataToList)
    {
        _postData = postData;
    ...
    {
        return _postData.Create(post, QueryGenerator<Post>.InsertQuery(post));
    }
    ...
    {
        return _postData.Delete(post, QueryGenerator<Post>.DeleteQuery(post));
    }
    public int? Update(Post post)   //redigera sin post och lägg till is_edited i table
    {
        return _postData.Update(post, QueryGenerator<Post>.UpdateQuery(post));
    }
